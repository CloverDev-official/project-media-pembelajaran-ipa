...
<div>
    <nav
        ...
        <button @click="open = !open">
            <iconify-icon icon="stash:burger-classic" width="24" height="24"
                class="text-blue-500 p-1 hover:text-white text-shadow-2xs rounded-lg transition-all duration-150 hover:bg-blue-500 active:scale-95"></iconify-icon>
        </button>
    </nav>
    ...
    <div x-show="open" @click.outside="open = false"
        ...
        x-transition:enter-start="transform translate-x-full" x-transition:enter-end="translate-x"
        x-transition:leave="transition-transform duration-300 ease-in-out"
        x-transition:leave-start="translate-x" x-transition:leave-end="transform translate-x-full"
        ...
        <div class="p-6">
            ...
            <ul class="space-y-2 mt-8 capitalize">
                <!-- beranda -->
                <x-nav-link-mobile href="{{ route('beranda') }}" :active="request()->routeIs('beranda')"><iconify-icon
                        icon="material-symbols:dashboard-outline" class="text-xl"></iconify-icon>
                    beranda</x-nav-link-mobile>
                <!-- materi -->
