...
>
    ...
    <div class="relative bg-black">
        ...
        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent px-4 pb-2">
    ...
    <div class="bg-gradient-to-r from-gray-800 to-gray-900 p-4">
        <div class="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-4">
            ...
            <!-- Kontrol Volume -->
            <div class="flex items-center justify-center space-x-2 px-3 py-2.5 sm:px-4 sm:py-3 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors">
        ...
        <div class="mt-3 text-center text-gray-300 text-sm" x-show="interactiveQuestions.length > 0">
    ...
    <div 
        ...
        style="display: none;"
    >
        <div class="bg-white rounded-xl w-full max-w-2xl h-full max-h-[90vh] flex flex-col shadow-2xl">
            <div class="flex-grow overflow-y-auto p-4 sm:p-6">
                <div class="flex items-center justify-between mb-4 pb-4 border-b">
                    <div class="flex items-center space-x-2">
                        ...
                        <h3 class="text-lg sm:text-xl font-bold text-gray-800">Pertanyaan Interaktif</h3>
                    </div>
                </div>

                <p class="text-base sm:text-lg font-semibold mb-6 text-gray-700 break-words" x-text="currentQuestion?.question"></p>
                
                <div class="space-y-3">
                    <template x-for="(option, key) in {a: currentQuestion?.option_a, b: currentQuestion?.option_b, c: currentQuestion?.option_c, d: currentQuestion?.option_d}" :key="key">
                        <button
                            ...
                            :class="{
                                ...
                                'border-green-500 bg-green-50 shadow-lg': answers[currentQuestion?.id] && answers[currentQuestion?.id].selected_answer === key && answers[currentQuestion?.id].is_correct,
                                'border-red-500 bg-red-50': answers[currentQuestion?.id] && answers[currentQuestion?.id].selected_answer === key && !answers[currentQuestion?.id].is_correct,
                                // Jika belum dijawab, tampilkan status normal
                                'border-blue-500 bg-blue-50 shadow-md transform scale-[1.02]': selectedAnswer === key && !isCorrect && selectedAnswer !== null && !answers[currentQuestion?.id],
                                'border-gray-300 hover:bg-gray-50 hover:border-gray-400': selectedAnswer !== key && selectedAnswer === null && !answers[currentQuestion?.id],
                                'opacity-50 cursor-not-allowed': answers[currentQuestion?.id] !== undefined
                            }"
            ...
            <div class="flex-shrink-0 p-4 bg-gray-50 border-t rounded-b-xl">
                <div x-show="answers[currentQuestion?.id]" 
                    ...
                    :class="answers[currentQuestion?.id]?.is_correct ? 'bg-green-100 border-2 border-green-500' : 'bg-red-100 border-2 border-red-500'">
                    <div class="flex-shrink-0">
                        <i :class="answers[currentQuestion?.id]?.is_correct ? 'fas fa-check-circle text-green-600 text-xl sm:text-2xl' : 'fas fa-times-circle text-red-600 text-xl sm:text-2xl'"></i>
                    </div>
                    <div>
                        <span class="font-bold" :class="answers[currentQuestion?.id]?.is_correct ? 'text-green-800' : 'text-red-800'" 
                            x-text="answers[currentQuestion?.id]?.is_correct ? 'Jawaban Benar!' : 'Jawaban Salah!'"></span>
                        <p x-show="!answers[currentQuestion?.id]?.is_correct" class="text-sm text-red-700 mt-1">
                            Jawaban yang benar: <span class="font-bold" x-text="currentQuestion?.correct_answer?.toUpperCase()"></span>
                        </p>
                    </div>
                </div>
    ...
    >
        <div class="bg-white rounded-xl p-6 sm:p-8 max-w-md w-full text-center shadow-2xl">
            ...
            <p class="text-sm text-gray-500 mb-6">
                <span x-text="correctAnswersCount"></span> jawaban benar dari <span x-text="totalQuestionsCount"></span> pertanyaan.
            </p>
            
            <!-- TOMBOL TUTUP -->
            <button 
                @click="closeScoreOverlay()"
