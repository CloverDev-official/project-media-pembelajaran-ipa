...
function showQuestion(index) {
  ...
  // kalau belum pernah diacak, acak sekali dan simpan
  if (!q.shuffledOptions) {
    ...
    // simpan hasil shuffle + jawaban benar baru
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 md:gap-4">
      ${q.shuffledOptions.map((opt, i) => {
        ...
        const btnClass = isSelected
          ? "option-btn py-5 px-5 w-[15rem] md:h-[10rem] rounded-lg border-2 font-bold text-xl text-center transition-all active:scale-95 bg-green-600 text-white border-black"
          : `option-btn py-5 px-5 w-[15rem] md:h-[10rem] rounded-lg border-2 font-bold text-xl text-center transition-all active:scale-95 ${optionColors[i]} text-white hover:opacity-80`;
  ...
  if (index === quizData.length - 1) {
    ...
    nextBtn.classList.replace(
      "bg-linear-to-t from-blue-600 to-blue-500 border-b-4 border-blue-700",
      "bg-linear-to-t from-red-600 to-red-500 border-b-4 border-red-700"
    );
  } else {
...
function selectAnswer(qIndex, optIndex, btn) {
  ...
  buttons.forEach((b, idx) => {
    b.className = `option-btn py-5 px-5 w-[15rem] md:h-[10rem] rounded-lg border-2 font-bold text-xl text-center transition-all active:scale-95 ${optionColors[idx]} text-white hover:opacity-80`;
  });

  // pilih jadi hijau
  btn.className = `option-btn py-5 px-5 w-[15rem] md:h-[10rem] rounded-lg border-2 font-bold text-xl text-center transition-all active:scale-95 bg-green-600 text-white border-black`;
...
function updateTimer() {
  ...
  timeLeft--; // kurangi setelah ditampilkan
}

// mulai quiz
function startQuiz() {
  quizData = shuffleArray(quizData); // acak soal
  ...
  updateTimer(); // 🔹 tampilkan waktu langsung
